                ...
                "capabilities": [
                    {
                        ...
                        "state": {"instance": "channel", "value": self._channel},
                    }
                ],
            },
        }
...
class ScenarioStepItemRequestedDeviceTTS(ScenarioStepItemRequestedDevice):
    ...
    def _value(self) -> ConfigType:
        return {
            ...
            "state": {"instance": "tts", "value": {"text": self._text}},
        }
...
class ScenarioStepItemRequestedDeviceTTSPA(ScenarioStepItemRequestedDevice):
    ...
    def _value(self) -> ConfigType:
        return {
            ...
            "state": {"instance": "phrase_action", "value": self._text},
        }
...
class ScenarioStepItemRequestedDeviceTextAction(ScenarioStepItemRequestedDevice):
    ...
    def _value(self) -> ConfigType:
        return {
            "type": "devices.capabilities.quasar.server_action",
            "state": {"instance": "text_action", "value": self._command},
        }


class ScenarioStepItemRequestedDeviceStop(ScenarioStepItemRequestedDevice):
    """Останавливает все действия на устройстве (stop_all)."""

    @property
    def _value(self) -> ConfigType:
        return {
            "type": "devices.capabilities.quasar",
            "state": {"instance": "stop_everything", "value": {}},
        }
    ...
    async def async_add_or_update_intent(
        ...
        if intent_player_device:
            step_items: list[ScenarioStepItem] = [
                ScenarioStepItemDeviceChannel(intent_player_device, intent.id)
            ]
            ...
            if intent.say_phrase:
                steps.append(ScenarioStep(ScenarioStepItemRequestedDeviceTTS(intent.say_phrase)))

            if intent.execute_command:
                commands = intent.execute_command
                if not isinstance(commands, list):
                    commands = [commands]
                
                for cmd in commands:
                    _LOGGER.debug("RAW CMD: %s (%s)", cmd, type(cmd))
            
                    # Рендерим команду
                    if isinstance(cmd, Template):
                        cmd_text = cmd.async_render(variables={}, parse_result=False)
                    else:
                        cmd_text = str(cmd)
            
                    _LOGGER.debug("RENDERED CMD: %s", cmd_text)
            
                    # Проверка stop_all ПОСЛЕ рендера
                    if "stop_all" in cmd_text:
                        steps.append(
                            ScenarioStep(ScenarioStepItemRequestedDeviceStop())
                        )
                        continue
            
                    steps.append(
                        ScenarioStep(
                            ScenarioStepItemRequestedDeviceTextAction(cmd_text)
                        )
                    )
                ...
                #     print(cmd)
                #     if "stop_all" in cmd:
                #         steps.append(ScenarioStep(ScenarioStepItemRequestedDeviceStop()))
                #         continue
                #     if hasattr(cmd, "async_render") and callable(cmd.async_render):
                #         cmd_text = cmd.async_render(variables={})
                #     elif hasattr(cmd, "template"):
                #         cmd_text = str(cmd.template)
                #     else:
                #         cmd_text = str(cmd)
                #     steps.append(ScenarioStep(ScenarioStepItemRequestedDeviceTextAction(cmd_text)))
