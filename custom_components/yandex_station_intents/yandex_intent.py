...
class Intent:
    ...
    execute_command: list[str | Template] | str | None = None
    ...
    def scenario_text_command(self) -> str:
        if isinstance(self.execute_command, list) and self.execute_command:
            cmd = self.execute_command[0]
        else:
            cmd = self.execute_command

        if isinstance(cmd, Template):
            if hasattr(cmd, "hass") and cmd.hass:
                return cmd.async_render()
            return str(cmd.template)
        elif cmd:
            return str(cmd)

        return str(self.say_phrase or "Команда")
...
class IntentManager:
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry, intents_config: ConfigType) -> None:
        ...
        for idx, name in enumerate(sorted(intents_config.keys(), key=lambda k: k.lower()), 0):
            ...
            if execute_command is None:
                commands_list: list[str | Template] = []
            elif isinstance(execute_command, list):
                commands_list = execute_command
            else:
                commands_list = [execute_command]
    ...
    async def _execute_command(
        ...
        yandex_station_entity_id: str,
    ) -> None:
        if self._detect_command_loop():
            return

        for command in intent.execute_command:
            if isinstance(command, Template):
                command.hass = self._hass
                content = await command.async_render(variables={"event": event_data})
            else:
                content = str(command)
            ...
            await self._hass.services.async_call(
                ...
                media_player.SERVICE_PLAY_MEDIA,
                {
                    ...
                    media_player.ATTR_MEDIA_CONTENT_ID: content,
                },
            )
    ...
    async def _tts(self, intent: Intent, event_data: ConfigType, yandex_station_entity_id: str) -> None:
        ...
        await self._hass.services.async_call(
            ...
            media_player.SERVICE_PLAY_MEDIA,
            {
                ...
                media_player.ATTR_MEDIA_CONTENT_ID: await intent.say_phrase_template.async_render(
