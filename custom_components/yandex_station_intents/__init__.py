...
def template_or_list(value):
    """template | list[template]"""
    if isinstance(value, list):
        return [string_or_template(v) for v in value]
    return string_or_template(value)


def intents_config_validate(intents_config: ConfigType) -> ConfigType:
    names = set(map(lambda s: s.lower(), intents_config.keys()))

    execute_commands: set[str] = set()

    for cfg in intents_config.values():
        cmd = cfg.get(CONF_INTENT_EXECUTE_COMMAND)
        if not cmd:
            continue
        ...
        for item in commands:
            if isinstance(item, template_helper.Template):
                execute_commands.add(item.template.lower())

    forbidden_phrases = execute_commands & names
    if forbidden_phrases:
        raise vol.Invalid(
            f"Недопустимо использовать команды в активационных фразах: {forbidden_phrases}"
        )
    ...
    for name, intent_config in intents_config.items():
        if (
            ...
            raise vol.Invalid(
                f"Недопустимо совместное использование execute_command "
                f"и шаблонной say_phrase в {name!r}"
            )
                                ...
                                vol.Schema(
                                    {
                                        ...
                                        vol.Optional(CONF_INTENT_EXECUTE_COMMAND): vol.Any(
                                            cv.template,
                                            [cv.template],
                                        ),
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry_data.connection_mode == ConnectionMode.DEVICE:
        ...
        entry.async_on_unload(
            hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP, event_stream.disconnect)
        )

    if entry_data.autosync:
        hass.loop.create_task(
            _async_setup_intents(manager.intents, quasar, intent_player_device)
        )
